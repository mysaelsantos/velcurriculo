...
interface ResumeFormProps {
  ...
  appliedCoupon: { code: string, type: 'fixed' | 'percentage', value: number, discount: number } | null;
  couponError: string;
...
const ResumeForm: React.FC<ResumeFormProps> = ({
  data, setData, isDemoMode, onStartEditing, onRequestPayment, isPaymentProcessing,
  onRequestDelete, hasPaidInSession, isEditing, showToast, currentStep, setCurrentStep,
  isFinished, setIsFinished, onRequestImport,
  // Props do sistema de cupom
  couponCode, setCouponCode, appliedCoupon, couponError, isValidatingCoupon,
  onValidateCoupon, onRemoveCoupon, paymentAmount
}) => {
  ...
  const [showSecondPhone, setShowSecondPhone] = useState(!!data.personalInfo.whatsappPhone);
        ...
        case 1:
          content = (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <input
                    ...
                    maxLength={15}
                  />
                  {/* Botão de adicionar segundo telefone */}
                  {!showSecondPhone && !data.personalInfo.whatsappPhone && (
                    <button
                      type="button"
                  ...
                  <div className="animate-fade-in">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full p-2 pl-10 pr-10 border rounded-md bg-white border-gray-300 text-gray-900 border-green-200 focus:border-green-400 focus:ring-green-400"
                        value={data.personalInfo.whatsappPhone || ''}
                        onChange={e => handlePersonalInfoChange('whatsappPhone', formatPhoneNumber(e.target.value))}
                        maxLength={15}
                      />
                      {/* Botão de remover segundo telefone */}
                      <button
                        type="button"
                        onClick={() => {
                          handlePersonalInfoChange('whatsappPhone', '');
                    ...
                    <p className="text-xs text-green-600 mt-1 pl-1">Este número será usado no QR Code do currículo</p>
                  </div>
                )}
              </div>

              <div className="flex items-center justify-between mt-2">
                <label htmlFor="show-whatsapp-qr" className="text-sm font-medium text-gray-700 pl-1">Exibir QR Code WhatsApp</label>
              ...
              <input type="text" placeholder="Link do LinkedIn (Opcional)" className="w-full p-2 border rounded-md bg-white border-gray-300 text-gray-900" value={data.personalInfo.linkedin || ''} onChange={e => handlePersonalInfoChange('linkedin', e.target.value)} />
              <div className="flex items-center justify-between mt-2">
                <label htmlFor="show-linkedin-qr" className="text-sm font-medium text-gray-700 pl-1">Exibir QR Code LinkedIn</label>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" id="show-linkedin-qr" className="sr-only peer" checked={data.style.showLinkedinQr} onChange={e => handleStyleChange('showLinkedinQr', e.target.checked)} />
                  <div className="w-10 h-5 bg-gray-300 rounded-full peer-checked:bg-blue-700 transition-colors duration-200"></div>
                  <div className="absolute left-0.5 top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 peer-checked:translate-x-5"></div>
                </label>
              </div>
  ...
  return (
    <React.Fragment>
      ...
      <div id="form-wizard" style={{ scrollMarginTop: '9rem' }} className="w-full lg:min-w-[350px] lg:max-w-[400px] lg:flex-shrink-0 lg:flex lg:flex-col bg-white p-6 rounded-lg shadow-md form-container">
        {!isFinished && (
                    ...
                    <div className="flex items-center justify-between bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl px-4 py-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-mono font-bold text-green-700 truncate">{appliedCoupon.code}</p>
