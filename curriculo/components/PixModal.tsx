...
const PixModal: React.FC<PixModalProps> = ({ isOpen, onClose, paymentData, onPaymentSuccess, isTestMode = false, amount }) => {
  ...
  const checkPaymentStatus = async (currentId: string) => {
    try {
      // console.log("Verificando status para:", currentId); 
      const backendUrl = `/.netlify/functions/get-payment-status?paymentId=${currentId}`;
            ...
            try {
                ...
                if (remaining > -120) { 
                    ...
                    if (!hasCheckedOnMount.current && status !== 'success') {
                        ...
                        checkPaymentStatus(parsedSession.data.paymentId).then(paid => {
                            if (!paid && remaining <= 0) {
  ...
  useEffect(() => {
    ...
    const timerInterval = setInterval(() => {
        ...
        if (remaining <= 0) {
            ...
            checkPaymentStatus(activePaymentData.paymentId).then(paid => {
                if (!paid) {
                    setStatus('expired');
                    clearSession();
  ...
  useEffect(() => {
    ...
    if (isOpen && status === 'pending' && !isTestMode) {
      intervalRef.current = setInterval(() => {
          checkPaymentStatus(activePaymentData.paymentId);
      }, 3000);
