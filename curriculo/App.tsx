...
const App: React.FC = () => {
    ...
    // Esta é a função com a correção de 'await document.fonts.ready;'
    const paginateResume = useCallback(async (dataToPaginate: ResumeData) => {
        
        // Esta é a linha que corrige o bug de "encolhimento"
        // Ela garante que as fontes sejam carregadas ANTES da medição
        if (document.fonts) {
    ...
    const exportToPdf = useCallback(async (dataToExport: ResumeData) => {
        ...
        // A espera pelas fontes já acontece dentro do 'paginateResume'
        // Mas é bom garantir aqui também, como no código antigo.
        ...
        // ***** INÍCIO DA CORREÇÃO *****
        // Em vez de recalcular, usamos o estado 'paginatedData'
        // que já foi calculado corretamente pelo 'useEffect'
        // (que por sua vez já esperou pelas fontes).
        const pagesToExport = paginatedData; 
        // ***** FIM DA CORREÇÃO *****
        ...
        try {
            ...
            for (let i = 0; i < pagesToExport.length; i++) {
                ...
                await new Promise<void>(resolve => {
                    ...
                    // O timeout de 300ms é mantido para garantir 
                    // que o React pinte tudo antes do html2canvas
            ...
            setIsPaymentProcessing(false);
            // Não precisamos mais chamar paginateResume() aqui, 
            // pois 'dataToExport' não foi alterada.
            // paginateResume(resumeData); // <--- Linha removida
        }
    }, [paginateResume, resumeData, isDemoMode, paginatedData]); // <--- 'paginatedData' foi adicionado às dependências
