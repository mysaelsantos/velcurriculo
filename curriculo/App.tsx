...
const App: React.FC = () => {
    ...
    // Alterado: Controla todo o estado de carregamento inicial
    const [isLoading, setIsLoading] = useState(true);
    const [generatingStatus, setGeneratingStatus] = useState<string>('');
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    
    // --- ESTADO PARA O GREETING DO HEADER ---
    const [showLogo, setShowLogo] = useState(true); // Logo começa visível
    const [headerMessage, setHeaderMessage] = useState('');
    const [hasGreeted, setHasGreeted] = useState(false); // Flag para executar apenas uma vez
    const [hasMotivatedEducation, setHasMotivatedEducation] = useState(false); // Flag para educação
    ...
    // --- LÓGICA DE CARREGAMENTO INICIAL COM DELAY ---
    useEffect(() => {
        const loadResources = async () => {
            // 1. Aguarda as fontes carregarem
            try {
            ...
            // 2. Aguarda um tempo mínimo (3.5 segundos) para garantir que tudo estabilize
            // e para o usuário ver a tela de boas-vindas
    ...
    useEffect(() => {
        // 0: Dados Pessoais, 1: Resumo, 2: Experiência, 3: Formação (Supondo index 3)
        // Se estiver no passo 3 (Formação) e ainda não motivou
        if (currentStep === 3 && !hasMotivatedEducation) {
             const timer = setTimeout(() => {
                 setHeaderMessage("Falta pouco agora!");
                 setShowLogo(false);

                 // Volta para a logo após 5 segundos
                 setTimeout(() => {
                     setShowLogo(true);
                     setHasMotivatedEducation(true); // Marca como concluído
                 }, 5000);

             }, 5000); // Delay inicial de 5s
    ...
    }, [resumeData, paginateResume, isLoading, isDemoMode]);
    ...
    }, [scalePreview, paginatedData, isLoading]);
    ...
    if (isLoading) {
        return (
            <div className="fixed inset-0 flex flex-col items-center justify-center bg-white z-[200]">
                ...
                {/* Spinner e Texto na parte inferior */}
