...
const App: React.FC = () => {
    ...
    // --- CORREÇÃO: Salva no localStorage sempre que resumeData mudar (não só nos passos) ---
    useEffect(() => {
    ...
    // --- CORREÇÃO: Renderização "silenciosa" com debounce para evitar lag ---
    useEffect(() => {
        if (measurementRootRef.current) {
            ...
            }, 50); // Pequeno atraso para agrupar renders
            return () => clearTimeout(timer);
        }
    }, [resumeData, isDemoMode]);

    // --- CORREÇÃO: ResizeObserver com debounce e cálculo dinâmico ---
    useEffect(() => {
        ...
        const handleResize = (entries: ResizeObserverEntry[]) => {
            ...
            debounceTimer = setTimeout(() => {
                // Dispara o cálculo da paginação após o layout estabilizar
    ...
    const calculatePagination = useCallback(async (dataToPaginate: ResumeData) => {
        ...
        // --- CÁLCULO DINÂMICO DA ZONA DE PERIGO ---
        ...
        const qrPadding = 40; // Buffer de segurança
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            if (overlapsDangerZone) {
                ...
                // Ajuste para garantir que o float limpe
