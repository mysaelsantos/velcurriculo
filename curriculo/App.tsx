...
const App: React.FC = () => {
    ...
    const paginateResume = useCallback(async (dataToPaginate: ResumeData) => {
        ...
        try {
            ...
            // INICIALIZAÇÃO SEGURA: Todos os campos opcionais inicializados para evitar crash no render
            ...
            const startNewPage = () => {
                ...
                // RESET SEGURO: Garante que todos os arrays existem
                currentPage = { 
                    personalInfo: dataToPaginate.personalInfo,
                    style: dataToPaginate.style,
                    experiences: [], education: [], courses: [], languages: [], skills: [],
            ...
            // Sempre empurra a última página, garantindo que não se perca dados
            // mesmo se for só uma página
            if (Object.keys(currentPage).length > 0) {
                // Checagem extra para garantir que não adicionamos uma página fantasma
                // se o loop terminou exatamente numa quebra de página
